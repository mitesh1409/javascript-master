...
It has 3 parts  
- **name (mandatory):** The function name.
- **parameters (optional):** The names of one or more arguments passed to the function. Maximum number of arguments varies in different engines.
- **statements (optional):** They comprise the body of the function.
...
It has 3 parts  
- **name (optional):** The function name. Can be omitted, in which case the function is anonymous. The name is only local to the function body. This means we can't access named function elsewhere.
- **parameters (optional):** The names of one or more arguments passed to the function.
- **statements (optional):** They comprise the body of the function.
...
// It receives function as a value later in the execution phase.
...
sayHello('Mitesh'); // Uncaught ReferenceError: sayHello is not defined
```

Here function `sayHello` is treated as a value, it is assigned to a variable `greet`.  
That is why it is not allocated any memory and hence cannot be accessed/used anywhere.  

Later on when we call `greet`, the underlying function `sayHello` is called.  
So its execution context is created - Memory Allocation + Code Execution.  
`sayHello` function execution context is pushed on top of the call stack.  
Memory is allocated to this function and that is why we can access this function inside its own body, in other words function is available locally inside the function body.

Execute this code and debug in Chrome browser console to better understand this.
...
when functions in that language are treated like any other variables.
