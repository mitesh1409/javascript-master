...
Each of the setTimeout callbacks in the for loop forms a closure with its parent (which is for loop block) where we defined `counter`.  
During each iteration the value of `counter` is incremented, and so each callback remembers its own copy of the `counter` variable using a closure with its parent scope (which is for loop block).
